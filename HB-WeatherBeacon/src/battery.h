...
//---------------------------------------------------------
template <uint8_t SENSPIN, uint16_t FACTOR> class BatteryResDiv : public Battery {
...
public:
    ...
    virtual ~BatteryResDiv() {}

    void init(uint32_t period, AlarmClock& clock)
    {
        pinMode(SENSPIN, INPUT);          // input
        digitalWrite(SENSPIN, LOW);       // pull-up off
        Battery::init(period, clock);
    }

private:
    virtual uint16_t voltage()
    {
        uint16_t adc = analogRead(SENSPIN); 

        if (adc) {
            uint32_t vbat = 11UL * adc * FACTOR / 1024 / 10;
            DPRINT(F("BatteryResDiv Voltage: "));
            DDECLN(vbat);
            return (uint16_t)vbat;
        } else {
            DPRINT(F("BatteryResDiv Error"));
            return 0;
        }
    }
};
